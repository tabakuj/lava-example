...
import { RelaySession, VRFData } from "./relay";
...
function createBaseMsgRelayPayment(): MsgRelayPayment {
  return { creator: "", relays: [], VRFs: [], descriptionString: "" };
}
...
export const MsgRelayPayment = {
  encode(message: MsgRelayPayment, writer: _m0.Writer = _m0.Writer.create()): _m0.Writer {
    ...
    for (const v of message.VRFs) {
      VRFData.encode(v!, writer.uint32(26).fork()).ldelim();
    }
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        ...
        case 3:
          ...
          message.VRFs.push(VRFData.decode(reader, reader.uint32()));
          continue;
        case 4:
          if (tag != 34) {
            break;
  ...
  fromJSON(object: any): MsgRelayPayment {
    return {
      ...
      VRFs: Array.isArray(object?.VRFs) ? object.VRFs.map((e: any) => VRFData.fromJSON(e)) : [],
  ...
  toJSON(message: MsgRelayPayment): unknown {
    ...
    if (message.VRFs) {
      obj.VRFs = message.VRFs.map((e) => e ? VRFData.toJSON(e) : undefined);
    } else {
      obj.VRFs = [];
    }
  ...
  fromPartial<I extends Exact<DeepPartial<MsgRelayPayment>, I>>(object: I): MsgRelayPayment {
    ...
    message.VRFs = object.VRFs?.map((e) => VRFData.fromPartial(e)) || [];
